                ...
                <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                        ...
                        <div className="p-6">
                            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 mb-6">
                                ...
                                <div className="space-y-2 text-gray-700">
                                    <div className="text-lg"><Latex>{'$f(x, y(x)) = y\' = -3x^2y$'}</Latex></div>
                            ...
                            <div className="mb-6">
                                ...
                                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                    <div className="text-sm space-y-2">
                                        <div><Latex>{'$k_1 = h \\cdot f(x_n, y_n) = h \\cdot (-3x_n^2 \\cdot y_n)$'}</Latex></div>
                                        <div><Latex>{'$y_{n+1} = y_n + h \\cdot f\\left(x_n + \\frac{h}{2}, y_n + \\frac{k_1}{2}\\right)$'}</Latex></div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div key={idx} className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-6 mb-4 border border-indigo-200">
                                    ...
                                    <div className="space-y-3 text-gray-700">
                                        <div className="bg-white p-3 rounded border border-indigo-100">
                                            ...
                                            <p className="ml-4"><Latex>{`$x_{${iter.n - 1}} = ${iter.x}$`}</Latex></p>
                                            <p className="ml-4"><Latex>{`$y_{${iter.n - 1}} = ${iter.y}$`}</Latex></p>
                                        </div>

                                        <div className="bg-white p-3 rounded border border-indigo-100">
                                            <p className="font-semibold text-indigo-700">Paso 1: Calcular k₁</p>
                                            <p className="ml-4 text-sm"><Latex>{`$f(x_{${iter.n - 1}}, y_{${iter.n - 1}}) = ${iter.f_n}$`}</Latex></p>
                                            <p className="font-mono ml-4 text-sm">k₁ = {iter.k1_calc}</p>
                                            <p className="ml-4"><Latex>{`$k_1 = ${iter.k1}$`}</Latex></p>
                                        </div>

                                        <div className="bg-white p-3 rounded border border-indigo-100">
                                            <p className="font-semibold text-indigo-700">Paso 2: Calcular punto medio</p>
                                            <p className="ml-4"><Latex>{`$x_{medio} = ${iter.x} + \\frac{${ejemploData.h}}{2} = ${iter.x_medio}$`}</Latex></p>
                                            <p className="ml-4"><Latex>{`$y_{medio} = ${iter.y} + \\frac{${iter.k1}}{2} = ${iter.y_medio}$`}</Latex></p>
                                        </div>
                                        ...
                                        <div className="bg-white p-3 rounded border border-indigo-100">
                                            ...
                                            <p className="ml-4"><Latex>{`$f_{medio} = ${iter.f_medio}$`}</Latex></p>
                                        </div>
                                        ...
                                        <div className="bg-indigo-100 p-3 rounded border-2 border-indigo-300">
                                            ...
                                            <p className="ml-4 text-sm"><Latex>{`$y_{${iter.n}} = ${iter.y} + ${ejemploData.h} \\times ${iter.f_medio}$`}</Latex></p>
                                            <p className="ml-4 text-xl font-bold text-indigo-700"><Latex>{`$y_{${iter.n}} = ${iter.y_next}$`}</Latex></p>
                                        </div>
                                    </div>
                                </div>
                            ))}
                    ...
                    {activeTab === 'calculadora' && (
                        <div className="p-6">
                            ...
                            {customResult && (
                                <div className="mt-6">
                                    {/* Ecuación Diferencial y Ordenada Genérica */}
                                    <div className="bg-purple-50 border border-purple-200 rounded-lg p-6 mb-6">
                                        <h3 className="text-xl font-bold text-purple-900 mb-4">Ecuación Diferencial:</h3>
                                        <div className="text-lg mb-4">
                                            <Latex>{`$y' = f(x,y) = ${convertirALatex(customInput.equation)}$`}</Latex>
                                        </div>

                                        <h3 className="text-xl font-bold text-purple-900 mb-4 mt-6">Ordenada Genérica:</h3>
                                        <div className="bg-white p-4 rounded-lg border border-purple-200 space-y-2">
                                            <div><Latex>{`$k_1 = h \\cdot f(x_n, y_n) = ${customInput.h} \\cdot (${convertirALatex(customInput.equation)})$`}</Latex></div>
                                            <div><Latex>{'$y_{n+1} = y_n + h \\cdot f\\left(x_n + \\frac{h}{2}, y_n + \\frac{k_1}{2}\\right)$'}</Latex></div>
                                        </div>
                                    </div>
                                        ...
                                        <div key={idx} className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-lg p-4 mb-3 border border-gray-300">
                                            ...
                                            <div className="space-y-2 text-sm">
                                                ...
                                                <div className="bg-white p-2 rounded">
                                                    ...
                                                    <p className="ml-4"><Latex>{`$f(x_{${iter.n - 1}}, y_{${iter.n - 1}}) = ${iter.f_n}$`}</Latex></p>
                                                    <p className="ml-4"><Latex>{`$k_1 = h \\times f = ${customInput.h} \\times ${iter.f_n} = ${iter.k1}$`}</Latex></p>
                                                </div>

                                                <div className="bg-white p-2 rounded">
                                                    <p className="font-semibold text-gray-700">Paso 2: Punto medio</p>
                                                    <p className="ml-4"><Latex>{`$x_{medio} = ${iter.x_medio}$`}</Latex></p>
                                                    <p className="ml-4"><Latex>{`$y_{medio} = ${iter.y_medio}$`}</Latex></p>
                                                </div>
                                                ...
                                                <div className="bg-indigo-100 p-2 rounded border-2 border-indigo-400">
                                                    ...
                                                    <p className="ml-4 text-lg font-bold text-indigo-700"><Latex>{`$y_{${iter.n}} = ${iter.y_next}$`}</Latex></p>
                                                    <p className="ml-4 text-sm text-gray-600"><Latex>{`$x_{${iter.n}} = ${iter.x_next}$`}</Latex></p>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
